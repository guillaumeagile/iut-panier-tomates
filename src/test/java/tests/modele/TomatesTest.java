...
public class TomatesTest {
    ...
    public void testRecuperationTomatesPremiereEtDerniere() {
        assertEquals("Tomate Russian Persimmon", tomates.getTomates().get(0).getDésignation());
        assertEquals(15, tomates.getTomates().get(0).getNbGrainesParSachet());
        assertEquals(4.93F, tomates.getTomates().get(0).getPrixTTC(), 0F);
        assertEquals(10, tomates.getTomates().get(0).getStock());
        assertEquals("Tomate Grosse Hâtive d'Orléans", tomates.getTomates().get(62).getDésignation());
        assertEquals(Couleur.ROUGE, tomates.getTomates().get(62).getCouleur());
        assertEquals("Variété Locale de la Région Centre-Val de Loire", tomates.getTomates().get(62).getSousTitre());
    }
    ...
    public void testGetterTomateParDésignation() {
        ...
        assertEquals("Tomate Brandywine", tomates.getTomate("Tomate Brandywine").getDésignation());
    }
    ...
    public void testFiltreTomatesCerises() {
        assertEquals(16, tomates.tomatesDeType(TypeTomate.TOMATES_CERISES).size());
    }

    @Test
    public void testFiltreParCouleur() {
        for (int i = 0; i < Couleur.values().length; i++) {
            List<Tomate> tomatesCouleur = tomates.tomatesDeCouleur(Couleur.values()[i]);
    ...
    public void testFiltresTomatesBasiquesParCouleur() {
        for (int i = 0; i < Couleur.values().length; i++) {
            List<Tomate> tomatesCouleur = tomates.tomatesDetypeDeCouleur(TypeTomate.TOMATES, Couleur.values()[i]);
            assertTrue(tomatesCouleur.size() > 0);
            for (Tomate tomate : tomatesCouleur) {
    ...
    public void testFiltresTomatesCerisesParCouleur() {
        for (int i = 0; i < Couleur.values().length; i++) {
            List<Tomate> tomatesCerisesCouleur = tomates.tomatesDetypeDeCouleur(TypeTomate.TOMATES_CERISES, Couleur.values()[i]);
