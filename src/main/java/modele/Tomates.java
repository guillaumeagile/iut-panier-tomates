...
public class Tomates {
     ...
     */
     private static List<Tomate> tomatesDeTypeAvecListe(TypeTomate typeTomate,
            List<Tomate> tomates) {
        List<Tomate> tomatesParTypeDeGraine = new LinkedList<Tomate>();
        for (Tomate t : tomates) {
            if (t.getType() == typeTomate) {
                tomatesParTypeDeGraine.add(t);
            }
        }
        return tomatesParTypeDeGraine;
    }
    ...
    private static List<Tomate> tomatesDeCouleurAvecListe(Couleur couleur,
            List<Tomate> tomates) {
        List<Tomate> tomatesParCouleur = new LinkedList<Tomate>();
        for (Tomate t : tomates) {
            if (t.getCouleur() == couleur) {
                tomatesParCouleur.add(t);
            }
        }
        return tomatesParCouleur;
    }
    ...
    public List<Tomate> tomatesDetypeDeCouleur(TypeTomate typeTomate,
            Couleur couleur) {
        List<Tomate> tomatesFiltrées = tomatesDeTypeAvecListe(typeTomate,
                this.tomates);
        return tomatesDeCouleurAvecListe(couleur, tomatesFiltrées);
    }
